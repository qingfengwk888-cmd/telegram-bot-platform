...
async def list_bots_by_tenant_id(tenant_id: str) -> List[dict]:
    ...
    bot_ids = sorted(list(await redis_client.smembers(tenant_bots_key(tenant_id)) or []))
    if not bot_ids:
        logger.info(
            "perf list_bots_by_tenant_id tenant_id=%s bot_ids=0 loaded=0 cost_ms=%s",
            tenant_id,
            cost_ms(start_ts),
        )
        return []

    keys = [bot_key(bot_id) for bot_id in bot_ids]
    raw_list = await redis_client.mget(keys)

    bots: List[dict] = []
    invalid_bot_ids: List[str] = []

    for bot_id, raw in zip(bot_ids, raw_list):
        if not raw:
            invalid_bot_ids.append(bot_id)
            continue

        try:
            bot = json.loads(raw)
        except Exception:
            logger.exception("failed parsing bot json tenant_id=%s bot_id=%s", tenant_id, bot_id)
            invalid_bot_ids.append(bot_id)
            continue

        if not bot.get("botId"):
            bot["botId"] = bot_id

        bot_status = str(bot.get("status") or "active")
        logger.info(
            "perf list_bots_by_tenant_id item tenant_id=%s bot_id=%s found=%s status=%s",
            tenant_id,
            bot_id,
            True,
            bot_status,
    ...
    if invalid_bot_ids:
        await redis_client.srem(tenant_bots_key(tenant_id), *invalid_bot_ids)

    logger.info(
        "perf list_bots_by_tenant_id tenant_id=%s bot_ids=%s loaded=%s cost_ms=%s",
        tenant_id,
        len(bot_ids),
        len(bots),
        cost_ms(start_ts),
...
async def list_all_bots_by_tenant_id(tenant_id: str) -> List[dict]:
    ...
    bot_ids = sorted(list(await redis_client.smembers(tenant_all_bots_key(tenant_id)) or []))
    if not bot_ids:
        logger.info(
            "perf list_all_bots_by_tenant_id tenant_id=%s bot_ids=0 loaded=0 cost_ms=%s",
            tenant_id,
            cost_ms(start_ts),
        )
        return []
    ...
    if invalid_bot_ids:
        await redis_client.srem(tenant_all_bots_key(tenant_id), *invalid_bot_ids)

    logger.info(
        "perf list_all_bots_by_tenant_id tenant_id=%s bot_ids=%s loaded=%s cost_ms=%s",
        tenant_id,
        len(bot_ids),
        len(bots),
        cost_ms(start_ts),
...
async def list_started_users_by_tenant_id_for_admin(tenant_id: str) -> List[dict]:
    ...
    tenant_user_ids = await redis_client.smembers(tenant_started_users_key(tenant_id))
    bots = await list_all_bots_by_tenant_id(tenant_id)
    ...
    if tenant_user_ids:
        for bot in bots:
            ...
            user_ids = await redis_client.smembers(bot_started_users_key(bot_id))
            hit_user_ids = [int(uid) for uid in user_ids if uid in tenant_user_ids]
            if not hit_user_ids:
                continue

            keys = [bot_user_profile_key(bot_id, uid) for uid in hit_user_ids]
            raw_list = await redis_client.mget(keys)
            ...
            for raw in raw_list:
                ...
                try:
                    ...
                    logger.exception(
                        "failed parsing tenant started user profile for admin tenant_id=%s bot_id=%s",
                        tenant_id,
                        bot_id,
                    )
                    continue
                ...
                results.append({
                    **u,
                    ...
                    "tenantId": str(u.get("tenantId") or tenant_id).strip(),
                    "botStatus": str(bot.get("status") or "active"),
                })

        results.sort(key=lambda x: int(x.get("startedAt") or 0), reverse=True)

        logger.info(
            "perf list_started_users_by_tenant_id_for_admin tenant_id=%s users=%s cost_ms=%s source=tenant_cache",
            tenant_id,
            len(results),
            cost_ms(started),
        )
    ...
    logger.info(
        "perf list_started_users_by_tenant_id_for_admin tenant_id=%s bots=%s users=%s cost_ms=%s source=fallback",
        tenant_id,
        len(bots),
        len(results),
        cost_ms(started),
...
async def recompute_tenant_today_started_user_count(tenant_id: str) -> None:
    ...
    count = sum(
        1 for u in users
        if int(u.get("userId") or 0) and is_same_ymd_ts_ms(u.get("startedAt"), today_ymd)
    )
...
async def save_started_user_profile(bot_id: str, user: dict) -> None:
    ...
    key = bot_user_profile_key(bot_id, user_id)

    old_profile = await redis_get_json(key) or {}
    first_started_at = old_profile.get("startedAt") or user.get("startedAt") or now_ms()
    ...
    # 只有资料真的变化时才写 profile
    if merged_profile != old_profile:
        await redis_set_json(key, merged_profile)

    added = await redis_client.sadd(bot_started_users_key(bot_id), user_id)
    if added != 1:
        return
    ...
    await incr_bot_stat(bot_id, "startedUserCount", 1)

    if tenant_id:
        await redis_client.sadd(tenant_started_users_key(tenant_id), user_id)

        await incr_tenant_stat(tenant_id, "startedUserCount", 1)

        if is_same_ymd_ts_ms(first_started_at, today_ymd):
            await incr_tenant_stat(tenant_id, "todayStartedUserCount", 1)
...
async def pick_default_bot_for_tenant(tenant_id: str) -> Optional[dict]:
    ...
    if latest_bot_id:
        bot = await load_bot(latest_bot_id)
        if bot and str(bot.get("status") or "active") != "deleted":
            return bot
...
async def list_started_users_by_tenant_id(tenant_id: str) -> List[dict]:
    ...
    if tenant_user_ids:
        ...
        for bot in bots:
            ...
            for raw in raw_list:
                ...
                try:
                    ...
                    logger.exception("failed parsing tenant started user profile tenant_id=%s bot_id=%s", tenant_id, bot_id)
                    continue
        ...
        logger.info(
            "perf list_started_users_by_tenant_id tenant_id=%s users=%s cost_ms=%s source=tenant_cache",
            tenant_id,
            len(results),
            cost_ms(started),
        )
    ...
    logger.info(
        "perf list_started_users_by_tenant_id tenant_id=%s bots=%s users=%s cost_ms=%s source=fallback",
        tenant_id,
        len(bots),
        len(results),
        cost_ms(started),
...
async def list_started_users(bot_id: str) -> List[dict]:
    ...
    user_ids = await redis_client.smembers(bot_started_users_key(bot_id))
    uid_list = [int(x) for x in (user_ids or [])]

    if not uid_list:
        logger.info(
            "perf list_started_users bot_id=%s users=0 cost_ms=%s",
            bot_id,
            cost_ms(started),
        )
        return []

    keys = [bot_user_profile_key(bot_id, uid) for uid in uid_list]
    raw_list = await redis_client.mget(keys)
    ...
    for raw in raw_list:
        ...
        try:
            ...
            logger.exception("failed parsing started user profile bot_id=%s", bot_id)

    results.sort(key=lambda x: int(x.get("startedAt") or 0), reverse=True)

    logger.info(
        "perf list_started_users bot_id=%s users=%s cost_ms=%s",
        bot_id,
        len(results),
        cost_ms(started),
    )
...
async def refresh_tenant_latest_bot_id(tenant_id: str) -> None:
    bots = await list_bots_by_tenant_id(tenant_id)
    if not bots:
        await redis_client.delete(tenant_latest_bot_id_key(tenant_id))
        return

    latest_bot = max(bots, key=lambda x: int(x.get("createdAt") or 0))
    latest_bot_id = str(latest_bot.get("botId") or "").strip()

    if latest_bot_id:
        await redis_client.set(tenant_latest_bot_id_key(tenant_id), latest_bot_id)
...
async def set_platform_tenant_blacklisted(tenant_id: str, value: bool) -> None:
    key = platform_tenant_black_key(tenant_id)
    if value:
...
async def is_platform_tenant_blacklisted(tenant_id: str) -> bool:
    return bool(await redis_client.get(platform_tenant_black_key(tenant_id)))

async def set_bot_user_blacklisted(bot_id: str, user_id: int, value: bool) -> None:
    key = bot_user_black_key(bot_id, user_id)
    set_key = bot_user_blacklist_set_key(bot_id)

    if value:
        await redis_client.set(key, "1")
        added = await redis_client.sadd(set_key, int(user_id))
        if added == 1:
            await incr_bot_stat(bot_id, "blacklistedUserCount", 1)
    else:
        removed = await redis_client.srem(set_key, int(user_id))
        await redis_client.delete(key)
        if removed == 1:
            await incr_bot_stat(bot_id, "blacklistedUserCount", -1)
...
async def is_bot_user_blacklisted(bot_id: str, user_id: int) -> bool:
    return bool(await redis_client.get(bot_user_black_key(bot_id, user_id)))
...
async def list_blacklisted_users(bot_id: str) -> List[dict]:
    ...
    user_ids = await redis_client.smembers(bot_user_blacklist_set_key(bot_id))
    uid_list = sorted([int(x) for x in (user_ids or [])])

    if not uid_list:
        logger.info(
            "perf list_blacklisted_users bot_id=%s users=0 cost_ms=%s",
            bot_id,
            cost_ms(started),
        )
        return []
    ...
    results: List[dict] = []
    for uid, raw in zip(uid_list, raw_list):
        if not raw:
            results.append({
                "userId": uid,
                "username": "",
        ...
        try:
            ...
            logger.exception("failed parsing blacklisted user profile bot_id=%s user_id=%s", bot_id, uid)
            results.append({
                "userId": uid,
                "username": "",
                "firstName": "",
    ...
    logger.info(
        "perf list_blacklisted_users bot_id=%s users=%s cost_ms=%s",
        bot_id,
        len(results),
        cost_ms(started),
    )
    ...
    if exists:
        ...
        if exists_status == "deleted":
            ...
            if old_tenant_id and old_tenant_id != tenant_id:
                await redis_client.srem(tenant_bots_key(old_tenant_id), bot_id)
